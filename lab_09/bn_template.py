...
class Variable(object):
    ...
    def calculate_marginal_probability(self):
        ...
        The marginal probability for a node (P(X = x)) is computed by summing over all
        possible assignments of its parent nodes. For a node with parents:
            P(X = x) = Σ_parent_assignments [ P(X = x | parents) * P(parents) ]
        If the node has no parents, its marginal probability is simply the probability distribution
        provided in its probability table.

        This function must be called before any other calculations, as subsequent computations
        rely on these marginal probabilities.
        """

        # If this node's marginal probabilities have already been calculated,
        # exit early to avoid redundant work.
        if self.ready:
            return

        # Ensure that the marginal probabilities of all parent nodes are computed first.
        for parent in self.parents:
            parent.calculate_marginal_probability()

        # Initialize the marginal probabilities for the current node to zero.
        for i in range(len(self.marginal_probabilities)):
            self.marginal_probabilities[i] = 0.0

        # If there are no parents, the node is independent.
        if len(self.parents) == 0:
            # Extract the single probability distribution tuple from the table.
            dist = list(self.probability_table.values())[0]
            # Assign the distribution directly to the marginal probabilities.
            for i in range(len(self.marginal_probabilities)):
                self.marginal_probabilities[i] = dist[i]
        else:
            # For nodes with parents, iterate over each combination of parent assignments.
            for parent_assignments, dist in self.probability_table.items():
                # Calculate the probability of this specific combination of parent assignments.
                # Use get_marginal_probability for the parent since we want its overall probability,
                # not a conditional probability that requires additional parent values.

                # Using multiply_vector_elements
                '''
                parent_prob = multiply_vector_elements(
                    [parent.get_marginal_probability(assignment_val) for parent, assignment_val in
                     zip(self.parents, parent_assignments)]
                )
                '''

                parent_prob = 1.0
                for parent, assignment_val in zip(self.parents, parent_assignments):
                    parent_prob *= parent.get_marginal_probability(assignment_val)

                # For each possible assignment of this node, add the weighted probability.
                for i in range(len(self.marginal_probabilities)):
                    self.marginal_probabilities[i] += dist[i] * parent_prob

        # Mark the node as "ready" so that future calls won't re-calculate the marginals.
        self.ready = True
...
class BayesianNetwork(object):
    ...
    def set_variables(self, varList):
        ...
        for var in self.variables:
            self.varsMap[var.name] = var
        self.ready = False  # we need to re-calculate va
    ...
    def get_joint_probability(self, values):
        """ return the joint probability of the variables in 'values'.
            'values' is a dict: { varName: assignment }
        """
        joint_probability = 1.0
        for varName, val in values.items():
            var = self.varsMap[varName]
            # Now we want to extract the parents' assignments
            parent_assignments = self.sub_vals(var, values)
            # Multiply in P(var = val | parents = parent_assignments)
            joint_probability *= var.get_probability(val, parent_assignments)
